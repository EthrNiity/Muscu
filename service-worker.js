const CACHE_NAME = 'ethrniity-v6';
const APP_FILES = ['./index.html', './manifest.json', './icon-192.png', './icon-512.png'];
const CDN_FILES = ['https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.min.js', 'https://unpkg.com/@supabase/supabase-js@2'];
...
self.addEventListener('install', (e) => {
  ...
  // No skipWaiting here on purpose: a newly installed worker now stays in the
  // "waiting" state until the user explicitly chooses to update (via the
  // in-app banner), instead of silently taking over and reloading the page.
});
...
self.addEventListener('push', (e) => {
  let payload = { title: 'EthrNiity', body: 'Tu as une nouvelle notification.', icon: './icon-192.png' };
  ...
  e.waitUntil(
    self.registration.showNotification(payload.title, {
      ...
      tag: payload.tag || 'ethrniity-notification',
